...
const uint8_t ssd1306_init_sequence [] = {	// Initialization Sequence
	...
	0x81, 0xFF,		// Set contrast control register - 0x01 to 0xFF - Default: 0x3F
	...
	0x22, 0x00, 0x1f,	// Set Page Address (start,end) 0 - 63
...
void OLED_DrawLogo(bool inverted) {
		OLED_setpos(0, 0);                      // set cursor to display start
		I2C_start(OLED_ADDR);                   // start transmission to OLED
		I2C_write(OLED_DAT_MODE);               // set data mode
		for(uint16_t i=0; i!=128*8; i++) {
      I2C_write(inverted? ~logo_bmp[i] : logo_bmp[i]); // send Logo
      SSD1306_Buffer[cursorX + (cursorY / 8) * SSD1306_WIDTH] = inverted? ~logo_bmp[i] : logo_bmp[i];
      cursorX++;
      if(cursorX > 127){
          cursorX = 0;
          cursorY += 8;
